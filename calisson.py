...
def drawHex(n):
    ...
    for i in range(n + 1):
        lineproj([-n+i, 0, i], [n, 0, i], **opt)
        lineproj([0, -n+i, i], [0, n, i], **opt)
        lineproj([i, -n+i, 0], [i, n, 0], **opt)
        lineproj([-n+i, i, 0], [n, i, 0], **opt)
        lineproj([i, 0, -n+i], [i, 0, n], **opt)
        lineproj([0, i, -n+i], [0, i, n], **opt)
...
def encodeCube(jeu, i, j, k):
    ...
    for x, y, z, d in lar:
        if not (  # les arêtes suivantes sont le long du tracé de l'hexagone englobant->ne pas encoder
            (x == 0 and ((z == n and d == "y") or (y == n and d == "z")))
            or (y == 0 and ((x == n and d == "z") or (z == n and d == "x")))
            or (z == 0 and ((y == n and d == "x") or (x == n and d == "y")))
        ):
...
def placeSommet(xs, ys, zs, d, M):
    ...
    if d == "z":
        ...
        if (
            ...
            and 0 < ys < n
            and zs < n
            and ((M[xs - 1, ys, zs], M[xs, ys - 1, zs]) in ((0, 1), (1, 0)))
        ):
            return (False, M)  # échec
        ...
        if 0 < xs < n and 0 < ys < n and zs < n:
            Mp[xs - 1, ys, zs] = Mp[xs, ys - 1, zs] = max(
                Mp[xs - 1, ys, zs], Mp[xs, ys - 1, zs]
            )
    ...
    if d == "x":  # on permute le code de "z", dans le sens direct (x->y->z->x)
        ...
        if (
            ...
            and 0 < ys < n
            and 0 < zs < n
            and ((M[xs, ys - 1, zs], M[xs, ys, zs - 1]) in ((0, 1), (1, 0)))
        ):
            return (False, M)  # échec
        ...
        if xs < n and 0 < ys < n and 0 < zs < n:
            Mp[xs, ys - 1, zs] = Mp[xs, ys, zs - 1] = max(
                Mp[xs, ys - 1, zs], Mp[xs, ys, zs - 1]
            )
            if Mp[xs, ys - 1, zs] == 0:
                Mp[xs:, ys - 1 :, zs:] = 0
                Mp[xs:, ys:, zs - 1 :] = 0
    ...
    if d == "y":  # on permute le code de "z", dans le sens indirect (x<-y<-z<-x)
        ...
        if (
            ...
            and ys < n
            and 0 < zs < n
            and ((M[xs, ys, zs - 1], M[xs - 1, ys, zs]) in ((0, 1), (1, 0)))
        ):
            return (False, M)  # échec
        ...
        if 0 < xs < n and ys < n and 0 < zs < n:
            Mp[xs, ys, zs - 1] = Mp[xs - 1, ys, zs] = max(
                Mp[xs, ys, zs - 1], Mp[xs - 1, ys, zs]
            )
            if Mp[xs, ys, zs - 1] == 0:
                Mp[xs:, ys:, zs - 1 :] = 0
                Mp[xs - 1 :, ys:, zs:] = 0
...
def test_solver(enig, dim, trace=False, file=None, withAxis=False):
    """
    ...
    for s in lsol:
        ...
        if -1 in s:
            ...
            print(
                f"solution {ns} incomplète : il reste {nmu} cube(s) non déterminé(s) !"
            )

    # calcul de l'affichage
    cs = 5 if ns < 3 else 4
    draw_solutions(enig, dim, lsol, cs, file, withAxis)

    return lsol


# tracé des solutions
def draw_solutions(enigma, n, lSols, cellSize=4, file=None, withAxis=False):
    """
    ...
    if len(lSols) == 0:
        ...
        if nls < 3:  # subplots sur une seule ligne
            ...
            plt.figure(figsize=(cellSize * (1 + nls), cellSize), tight_layout=True)
        ...
        axs = plt.subplot(formatSubPlots, adjustable="box", aspect=1 / np.sqrt(3))
        if not withAxis:
            axs.axis("off")
        drawHex(n)
        draw_enigma(enigma)
        ...
        for ns in range(min(nls, 5)):
            axs = plt.subplot(
                formatSubPlots + ns + 1, adjustable="box", aspect=1 / np.sqrt(3)
            )
            if not withAxis:
                axs.axis("off")  
