...
def randomEnigma2(n, trace=False, easy=0):
    ...
    if trace:
        print(f"Empilement de taille {n} terminé ({np.sum(mat)} cubes)")
    ...
    if trace:
        print(f"Première passe : {len(lar3D)} arêtes")
    ...
    if trace:
        print(f"Ajout d'arêtes pour lever les indéterminations : {len(lar3D)} arêtes")
    ...
    if trace:
        print(f"ajout pour facilité {easy} -> {len(lar3D)} arêtes")
    ...
    if trace:
        print(f"énigme par empilement construite")
...
def randomEnigma_fromConstraints(n, trace=False, enig=[]):
    ...
    #    enig = []
    if trace:
        print(f"Génération d'une énigme de taille {n}")
    if trace:
        print("élimination des cubes indéterminés")

    while True:
        rs = doSolve(
            enig, n, filterFunc=lambda m: -1 in m
        )  # la liste des Résultats de la réSolution

        rsf = list(
            filter(lambda m: -1 in m, rs)
        )  # la liste des résultats contenant des cubes indéterminés
        ...
        if trace:
            print(f"nombre d'arêtes dans l'enigme : {len(enig)}")

    if trace:
        print(f"durée phase 1 : {time.monotonic()-start} s ({len(rs)} solution(s))")
    ...
    if trace:
        print("élimination des solutions multiples")
    while len(rs) > 1:
        # calcul de la différence ensembliste des arêtes
        ars = set(encodeSolution(encodage(rs[1]))) - set(
            encodeSolution(encodage(rs[0]))
        )
    ...
    if trace:
        print(
            f"nombre de cubes dans l'empilement : {nCubes} ({100 * nCubes / n**3:.1f} %)"
        )

    # fini : on a une solution unique !
    if trace:
        print(f"durée totale : {time.monotonic()-start} s")
